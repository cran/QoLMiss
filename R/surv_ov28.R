...
#'
#' @details surv_ov28 function inputs either a dataset containing missing information, represented as,
...
#'
#' @param x A data frame with ID, time, event, arm, OV_Q31,OV_Q32,...,OV_Q58 columns along with other columns if data is available.
#'
#' @import utils
#' @import dplyr
#' @import survival
...
surv_ov28 <- function(x){
  covariates <- c('Abdominal_GI','Peripheral_Neuropathy','Hormonal','Body_Image','Attitude_to_Disease','Chemotherapy_side_effects','Other_Single_Items','Sexuality')

  arm1_qol <- ovc_qol(x[x$arm==1,])
  arm2_qol <- ovc_qol(x[x$arm==2,])
